...
let allCaps = [];
runners.map((runners)=>{
  cap=runners.first_name.toUpperCase();
  allCaps.push(cap);
});
console.log(allCaps); 
...
ticketPriceTotal.push(runners.reduce((total, runners)=>{
...
// Problem 1: We want to know who are big donors are >200

let big_donors=runners.filter((runners)=>{
  return runners.donation > 200;
});

console.log(big_donors);

// Problem 2: We need to determine the companies that each of our runners work at to see if we can get sponsorships for the event 

let companies=runners.map((runners)=>{
  return runners.company_name;
});

console.log(companies.sort());

// Problem 3: We now also want to know all the last name, email address, and company that each persons work at

let sponsorship_outreach=[];
runners.forEach((runners)=>{
  sponsorship_outreach.push(runners.last_name + ", " + runners.email + ", " + runners.company_name);
});

console.log(sponsorship_outreach);